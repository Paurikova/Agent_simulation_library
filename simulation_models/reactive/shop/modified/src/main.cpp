//TODO include
#include "../include/Graph2ReactiveReasoning.h"
...
int main(int argc, char** argv) {{
    ...
    std::unique_ptr<Graph2ReactiveReasoning> rGraph2ReactiveReasoning = std::make_unique<Graph2ReactiveReasoning>(500, window, stateShop.get(), logger.get());
    std::unique_ptr<Agent> aGraph2 = std::make_unique<Agent>(6, aManager1.get(), rGraph2ReactiveReasoning.get());
    simulationCore->registerAgent(aGraph2.get());
    std::unique_ptr<Cash6ReactiveReasoning> rCash6ReactiveReasoning = std::make_unique<Cash6ReactiveReasoning>(30, 25, stateShop.get(),logger.get());
    std::unique_ptr<Agent> aCash6 = std::make_unique<Agent>(5, aManager1.get(), rCash6ReactiveReasoning.get());
    simulationCore->registerAgent(aCash6.get());
    std::unique_ptr<Line5ReactiveReasoning> rLine5ReactiveReasoning = std::make_unique<Line5ReactiveReasoning>(stateShop.get(), logger.get());
    std::unique_ptr<Agent> aLine5 = std::make_unique<Agent>(4, aManager1.get(), rLine5ReactiveReasoning.get());
    simulationCore->registerAgent(aLine5.get());
    std::unique_ptr<Customer4ReactiveReasoning> rCustomer4ReactiveReasoning = std::make_unique<Customer4ReactiveReasoning>(stateShop.get(), logger.get());
    std::unique_ptr<Agent> aCustomer4 = std::make_unique<Agent>(2, aManager1.get(), rCustomer4ReactiveReasoning.get());
